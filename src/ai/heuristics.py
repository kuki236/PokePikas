...
def evaluate_level3_state(state: BattleState, player_id: int) -> float:
    ...
    ko_score = 0.0
    if my_active.current_hp <= 0: ko_score -= 1.0

    return hp_score + (0.5 * ko_score)

def evaluate_level4_state(state: BattleState, player_id: int) -> float:

    ALPHA = 3.0   # [Vida Global]
    BETA  = 1.5   # [Velocidad]
    GAMMA = 2.0   # [Tipos Suavizados]
    DELTA = 0.5   # [Estado]
    EPSILON = 2.0 # [Diferencia de Pokémon Vivos]
    ...
    alive_score = (my_alive - opp_alive) / max(1, len(my_team))
    ...
    return (ALPHA * hp_score) + (BETA * speed_score) + (GAMMA * type_score) + (DELTA * status_score) + (EPSILON * alive_score)