...
class Level3Agent(BaseAgent):
    ...
    def _get_legal_actions(self, team: List[PokemonState], active_idx: int, turns_since_switch: int) -> List[Action]:
        actions = []
        ...
        if active.current_hp > 0 and getattr(active, 'moves', []):
            for i, move in enumerate(active.moves):
                if getattr(move, 'current_pp', 0) > 0:
                    actions.append(Action(type=ActionType.MOVE, target_index=i))
    ...
    def get_action(self, state: BattleState) -> Action:
        ...
        legal_actions = self._get_legal_actions(team, active_idx, self.turns_since_last_switch)
        ...
        for my_action in legal_actions:
            ...
            value = self._min_value(next_state, AI_LEVEL3_DEPTH - 1, alpha, beta)
    ...
    def _max_value(self, state: BattleState, depth: int, alpha: float, beta: float) -> float:
        if depth == 0 or self._is_match_over(state):
            return evaluate_level3_state(state, self.player_id)

        team, active_idx, _, _ = self._get_team_and_active(state)
        legal_actions = self._get_legal_actions(team, active_idx, 2)
    ...
    def _min_value(self, state: BattleState, depth: int, alpha: float, beta: float) -> float:
        """Capa MIN: El Oponente busca minimizar nuestra ventaja."""
        if depth == 0 or self._is_match_over(state):
            return evaluate_level3_state(state, self.player_id)

        _, _, opp_team, opp_active_idx = self._get_team_and_active(state)
        opp_actions = self._get_legal_actions(opp_team, opp_active_idx, 2)
